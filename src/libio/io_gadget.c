...
#define CHECK_FLOATBYTES(bfile, bstore) {                                           \
		...
		if (bfile < bstore) {                                                       \
			...
			io_logging_warn(log, INT32_C(                                           \
			                ...
			                "(%" PRIi32                                             \
			                " bytes vs. %" PRIi32                                   \
			                " bytes). Will downcast, but precision might be lost.", \
			                bfile, bstore);                                         \
		}                                                                           \
}
extern uint64_t
                       ...
                       io_file_strg_struct_t strg)
{
	...
	io_logging_msg(log, INT32_C(1),
	               "A total of %" PRIu64
	               " particle IDs with %" PRIi32
	               " bytes per int (%f MB total) "
	               "are stored.",
...
#undef BOOK_KEEPING
	...
	if (f->header->np[0] > 0) {
		...
		io_logging_msg(log, INT32_C(1),
		               "A total of %" PRIu64
		               " gas particle energies with %" PRIi32
	...
	/*******************************************************************
	 *  Read the smoothing scale                                       *
	 *******************************************************************/
	if (f->header->np[0] > 0) {
		if (f->ver != 2) {
			fprintf(
			    stderr,
			    "HSML *not* read, as it is a Gadget1 format and "
			    "searching for the block is not implemented for those.\n");
			return pread;
		}
		/* Start with the next block */
		DESCRIBE_BLOCK;
		while (strncmp(str, "HSML", 4) != 0) {
		...
		io_logging_msg(log, INT32_C(1),
		               "A total of %" PRIu64
		               " gas hsmls with %" PRIi32
		               " bytes per float (%f MB total) are stored.",
		               f->header->np[0], bytes_file,
		               (float)(blocksize / 1024. / 1024.));
		if ((pskip <= (uint64_t)(f->header->np[0]))
		    && (strg.hsml.val != NULL)) {
			fseek(f->file, partsize * pskip, SEEK_CUR);
			for (i = 0; i < f->header->np[0] - pskip && i < pread; i++) {
				if (bytes_file == sizeof(float)) {
					...
					io_util_readdouble(f->file, &fhsml, f->swapped);
				}
				/* STEP 2:  Store the energy in the array */
				if (strg.bytes_float == sizeof(float)) {
					*((float *)strg.hsml.val) = (float)fhsml;
				} else {
					*((double *)strg.hsml.val) = fhsml;
				}
				/* STEP 3:  Increment the pointers to the next particle */
				strg.hsml.val = (void *)(((char *)strg.hsml.val)
				                         + strg.hsml.stride);
			}
			/* Skip to the end of the energy block */
			fseek(f->file,
			      partsize * (f->header->np[0] - pskip - i),
			      SEEK_CUR);
		} else {
			i = 0;
			fseek(f->file, partsize * (f->header->np[0]), SEEK_CUR);
...
#define SCALE_CALL(type) {                                       \
		...
		*((type *)strg.momz.val) *= (type)(scale_mom);           \
		...
		if (strg.hsml.val != NULL)                               \
			*((type *)strg.hsml.val) *= (type)(scale_pos);       \
		strg.posx.val = (void *)(((char *)strg.posx.val)         \
		                         + strg.posx.stride);            \
		strg.posy.val = (void *)(((char *)strg.posy.val)         \
		                         + strg.posy.stride);            \
...
local_opengetswap(io_logging_t log, io_gadget_t f)
{
	...
	rewind(f->file);
	(void)fread((void *)(&tmp), sizeof(int32_t), 1, f->file);
