...
io_art_header_get(io_logging_t log, io_art_t f)
{
	...
	if (is == 0) {
		io_logging_fatal(log,
		                 "could not find the 's' in the input file name");
		return NULL;
	}
	...
	PMcrd[iloop] = '\0';
	fprintf(stderr,
	        "o reading header of size %d from file %s (slen=%d)\n",
	        (int)(sizeof(io_art_header_struct_t)),
	        PMcrd,
	        (int)(strlen(PMcrd)));
	...
	if (fPMcrd == NULL) {
		io_logging_fatal(log,
		                 "could not open PMcrd.DAT with ART header"
		                 " information");
		return NULL;
	}
	...
	for (iloop = 0; iloop < 45; iloop++) {
		fread(&(dummy->header_string[iloop]), sizeof(char), 1, fPMcrd);
	//	strcpy(&(dummy->header_string[iloop]), &c);
	}
	dummy->header_string[45] = '\0';
	...
	if (dummy->nspecies == 0) {
		dummy->N_particles = (dummy->nrowc) * (dummy->nrowc)
		                     * (dummy->nrowc);
		...
		dummy->N_pages
		                   = (dummy->N_particles
		       - 1) / ((dummy->nrowc) * (dummy->nrowc)) + 1;
		dummy->N_in_last   = dummy->N_particles
		                     - ((dummy->nrowc)
		                        * (dummy->nrowc)) * (dummy->N_pages - 1);
	}
	...
	if ((dummy->extras[100 - 1] > 0) && (dummy->extras[59 - 1] > 0)) {
		...
		if ((fart = fopen("art.info", "r")) == NULL) {
			io_logging_fatal(log,
			                 "Could not open art.info containing "
			                 "the following information:");
			io_logging_fatal(log,
			                 "20.0       ART_BOXSIZE     [Mpc/h]");
			io_logging_fatal(log,
			                 "2.2197e15  ART_MUNIT       [Msun/h]");
			io_logging_fatal(log,
			                 "Please generate this file. Aborting now!");
			free(dummy);
			return NULL;
		}
