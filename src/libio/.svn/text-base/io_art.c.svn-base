                ...
                io_file_strg_struct_t strg)
{
	...
	io_art_scale_global(log, f->mycomm, f->maxpos,
	                    f->minpos, &(f->mmass), &(f->minweight),
	                    &(f->maxweight), &(f->sumweight));
#endif
...
#define CHECK_FLOATBYTES(bfile, bstore) {                            \
		if (bfile == sizeof(float)) {                                \
			io_logging_msg(log, INT32_C(1),                          \
			               ...
			               "floating point values (%" PRIu32         \
			               " bytes).",                               \
			               bfile);                                   \
		} else if (bfile == sizeof(double)) {                        \
			io_logging_msg(log, INT32_C(1),                          \
			               "Obviously the file uses double for "     \
			               "floating point values (%" PRIi32         \
			               " bytes).",                               \
			               bfile);                                   \
		} else {                                                     \
			io_logging_fatal(log,                                    \
			                 "No clue what kind of floatin "         \
			                 "point uses %" PRIi32                   \
			                 " bytes. Aborting reading.",            \
			                 bfile);                                 \
			return UINT64_C(0);                                      \
		}                                                            \
		if (bfile < bstore) {                                        \
			io_logging_msg(log, INT32_C(1),                          \
			               "The floating point values in the file "  \
			               "have less precision than the particle "  \
			               "storage (%" PRIi32                       \
			               " bytes vs. %" PRIi32                     \
			               " bytes). No problem, will upcast.",      \
			               bfile, bstore);                           \
		} else if (bfile > bstore) {                                 \
			io_logging_warn(log, INT32_C(1),                         \
			                "The floating point values in the file " \
			                " have a higher precision than the "     \
			                " particle storage (%" PRIi32            \
			                "bytes vs. %" PRIi32                     \
			                " bytes). Will downcast, but precision " \
			                " might be lost.",                       \
			                bfile, bstore);                          \
		}                                                            \
}
...
#define BOOK_KEEPING {                                              \
		if (isgreater(fweight,                                      \
		              oldfweight) || isless(fweight, oldfweight)) { \
			f->no_species++;                                        \
			oldfweight = fweight;                                   \
                    ...
                    io_file_strg_struct_t strg)
{
	...
	io_logging_msg(log, INT32_C(1),
	               "A total of %" PRIu64
	               " particles with %" PRIi32
	               " bytes per float (%f MB total) are stored.",
	...
	// in the next page we now have to skip
	// (pskip-NpagesSkip*NPAGE) particles
	...
	while (preadIROW > 0 && IROW <= Npages) {
		...
		for (i = 0; i < preadIROW; i++) {
			...
			// positions are in range [1,ngridc] and hence subtract 1
			fposx                     = (double)(dummy - 1.0);
		...
		for (i = 0; i < preadIROW; i++) {
			...
			*((uint32_t *)strg.id.val) = (uint32_t)(Ipart);
		...
		for (i = 0; i < preadIROW; i++) {
			...
			if (isless(partw, f->minweight))
				f->minweight = fweight;
			if (isgreater(partw, f->maxweight))
				f->maxweight = fweight;

			*((float *)strg.weight.val) = (float)fweight;
			strg.weight.val             = (void *)
			                              (((char *)strg.weight.val)
			                   + strg.weight.stride);
